    ...
    public partial class frmBankPayment : Form
    {
        ...
        private void txtvoucherno_Validated(object sender, EventArgs e)
        {
            ...
            if (dr.HasRows == true)
            {
                ...
                drd = c.fillreader(ref drd, "select Amount from tbl_VoucherDetail where CashBankNo>0 and VchType='BP' and Vouchno='"+txtvoucherno.Text+"'", conn);
                if (drd.HasRows == true)
                {
                    drd.Read();
                    txtPaidAmt.Text = drd["Amount"].ToString();
                    drd.Close();
                }               
            }          
            else
