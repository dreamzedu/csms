...
namespace SMS
{
    public partial class frmBankBook : Form
    {
        ...
        {
            c.getconnstr();
            c.FillcomboBox("select BHM_DESH,GRP_COD,BHM_COD from tbl_LedgerAcc  where GRP_COD IN (2) order by BHM_DESH", "BHM_DESH", "BHM_COD", ref valcmbcash);
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //"select ISNULL(a.Receipt,0) as receipt,ISNULL(a.Payment ,0) as payment,a.balance  from  (SELECT SUM((case  when a.Amount < 0 then -1*a.Amount else 0 end)) AS Receipt,SUM((case  when a.Amount >= 0 then a.Amount else 0 end)) AS Payment,0 as balance  FROM tbl_VoucherDetail AS a  Where a.CashBankNo=0 and a.VchDate>='" + Cashopdate.Date + "' and  a.VchDate<'" + dtpfrom.Value.ToShortDateString() + "' and a.AccCode=" + valcmbcash.SelectedValue + " and a.YearNo=" + m.psessioncode + ") a ", con);
                decimal Cashrec = 0;
                decimal Cashpay = 0;
                if (dr.HasRows == true)
                {
                ...
                //mysql = mysql + "  values  ("+m.psessioncode+",'" + dtpfrom.Value + "' ,'Open. Bal.',0," + CashopBal + ","+CashopBal+")";
                ...
                c.returnconn(c.myconn);              
                trn = c.myconn.BeginTransaction();
                mysql = "insert into tbl_Ledger (yearno,accno,accname,vchno,vchdate,vchtype,remark,ChequeNo,ChequeDate,vchamt,Reciept,Payment) ";
                mysql = mysql + " SELECT 0 as yearno,a.AccCode as Accno,c.bhm_desh as accname, b.VouchNo, b.VchDate,b.VchType,remark ,a.ChequeNo,a.ChequeDate as Date,a.Amount as vchamt,";
                ...
                mysql = mysql + " FROM tbl_VoucherDetail AS a, tbl_Voucher AS b,tbl_LedgerAcc c  Where a.VchType in ('CV','BP','BR') and    "; 
                mysql = mysql + "  a.VchDate>='"+dtpfrom.Value.Date+"' and  a.VchDate<='"+dtpto.Value.Date+"'  and  a.YearNo='"+school.CurrentSessionCode+"' and   ";
                mysql = mysql + "  a.VouchNo  = b.VouchNo  and a.VchType = b.VchType and a.AccCode=c.GRP_COD and a.CashBankNo<>0  ";
                ...
                RptBankBook cr = new RptBankBook();
