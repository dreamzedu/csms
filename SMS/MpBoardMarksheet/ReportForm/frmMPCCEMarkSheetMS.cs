...
{
    public partial class frmMPCCEMarkSheetMS : UserControl
    {
        public frmMPCCEMarkSheetMS()
        {
            InitializeComponent(); Connection.SetUserControlTheme(this);
        }
        SqlDataReader dataReader = null;
        bool NonPrimaryFlage;
        private void frmMPCCEMarkSheetMS_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnBackView_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtStudentMarksDetail = Connection.GetDataTable("[dbo].[GetViewMSCCEMarksDetail] " + cmbClass.SelectedValue + "," + cmbScetion.SelectedValue + "," + school.CurrentSessionCode + "");
            DataTable dtStudentSkillsDetail = Connection.GetDataTable("Select (Case When t1.ExamCode=0 Then 'SA1_' When t1.ExamCode=1 Then 'SA2_' When t1.ExamCode=2 Then 'SA3_'  End) AS ExamCode , " +
                " t2.studentno StudentNo,t2.scholarno ScholarNo,t2.Name ,t1.SkillId ,t1.SkillName ,t1.Description ,dbo.GETGRADEMP(t1.Grade,18)  AS Grade  From ( " +
                " Select s2.ExamCode,s1.SkillId ,s1.SkillName ,s2.StudentNo ,s2.Description ,s2.Grade ,s2.ClassNo ,s2.SectionCode  From tbl_CCESkills s1,tbl_MSCCEStudentSkills s2 Where s1.SkillId =s2.SkillId And " +
            ...
            for (int r = 0; r < 1; r++)
            {
                ...
                dataReader = Connection.GetDataReader("SELECT SkillId, SUBSTRING( SkillName,0,24) as SkillName,  Status FROM tbl_CCESkills Order By SkillId");
                    ...
                    if (this.NonPrimaryFlage)
                    {
                        for (int Exam = 0; Exam < dtExam.Rows.Count; Exam++)
                        {
                            for (int skill = 0; skill < dtSkill.Rows.Count; skill++)
                            {
                                ...
                                if (dtSkill.Rows[skill][1].ToString().Length >= 23)
                                {
                                    NewRow[dtExam.Rows[Exam]["ExamCode"].ToString() + dtSkill.Rows[skill][1].ToString().Substring(0, 23)] = DI.TrimEnd(',');
                                    NewRow[dtExam.Rows[Exam]["ExamCode"].ToString() + dtSkill.Rows[skill][1].ToString().Substring(0, 23) + "_G"] = Grade;
                                }
                                else
                                {
            ...
            CrystalDecisions.CrystalReports.Engine.ReportDocument s = null;
            if (int.Parse(cmbClass.SelectedValue.ToString()) > 105)
            {
                s = new MpBoardMarksheet.ReportDesign.rptMSCCEReportFormatBack_S();
            }
            else
            {
                s = new MpBoardMarksheet.ReportDesign.rptMSCCEReportFormatBack();
            }
        ...
        private void frmMPCCEMarkSheetMS_Paint(object sender, PaintEventArgs e)
        {
