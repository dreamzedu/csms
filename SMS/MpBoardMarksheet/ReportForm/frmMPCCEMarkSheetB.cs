...
{
    public partial class frmMPCCEMarkSheetB : UserControl
    {
        public frmMPCCEMarkSheetB()
        {
            InitializeComponent(); Connection.SetUserControlTheme(this);
        }

        private void frmMPCCEMarkSheetB_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void frmMPCCEMarkSheetB_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void Btn_Format1_Click(object sender, EventArgs e)
        {
            ...
            for (int j = 17; j < dtStudentMarksDetail.Columns.Count; j++)
            {
            ...
            //for practical 9th and 11th class
            for (int r = 0; r < 1; r++)
            {
                ...
                for (int k = 1; k <= 10; k++)
                {
                    if (k <= 6)
                    {
                        ...
                        if (tmp1 == 6)
                        {
                            ...
                            for (int GetGrade = 17; GetGrade < dtStudentMarksDetail.Columns.Count; GetGrade++)
                            {
            ...
            ds.WriteXmlSchema(@"" + Connection.GetAccessPathId() + @"Barcodes\a\rptMPCBSEMarksBackDetailFormat_1.xsd");
            ...
            {
                s = new MpBoardMarksheet.ReportDesign.rptMPMarksheetFormat1();
            }
            else if ( cmbClass.Text.Trim().Equals("VI") || cmbClass.Text.Trim().Equals("VII") || cmbClass.Text.Trim().Equals("VIII"))
            {
                s = new MpBoardMarksheet.ReportDesign.rptMPMarksheetFormat2();
            }
            else
            {
                s = new MpBoardMarksheet.ReportDesign.rptMPMarksheetFormat_Nur_Fifth();
            }
